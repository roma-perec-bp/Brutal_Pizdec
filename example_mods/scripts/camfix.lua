...
function initializeCams()
	local function cam(t, cam, class)
		t.getProperty = type(class) == "string" and function(v) return getPropertyFromClass(class, cam .. "." .. v) end or function(v) return getProperty(cam .. "." .. v) end
		t.setProperty = type(class) == "string" and function(v, x) return setPropertyFromClass(class, cam .. "." .. v, x) end or function(v, x) return setProperty(cam .. "." .. v, x) end
...
function updateCam(t, dt)
	...
	local x = getProperty("camGame.scroll.x")
	if (t == cams.game and type(x) == "number") then
		setProperty("camGame.scroll.x", x + t.scrollOffset.x)
		setProperty("camGame.scroll.y", getProperty("camGame.scroll.y") + t.scrollOffset.x)
	else
...
function updateCamEarly(t, dt)
	if (t == cams.game) then
		setProperty("camGame.scroll.x", getProperty("camGame.scroll.x") - t._lastScrollOffset.x)
		setProperty("camGame.scroll.y", getProperty("camGame.scroll.y") - t._lastScrollOffset.y)
	else
